...
b_node_t *
b_node_create(void *data, size_t data_size)
{
	return __b_node_create(data, data_size);
}
...
static void
__b_tree_print_levelorder(b_tree_t *b_tree, void (*print_data)(void *))
{
	...
	while (!q_is_empty(q)) {
		...
		int nivel = q_get_size(q);

		for (int i = 0; i < nivel; i++) {
			b_node_tmp_addr = q_front(q);
			b_node_tmp = *b_node_tmp_addr;
			q_dequeue(q);
			print_data(b_node_tmp->data);
			if (b_node_tmp->left) {
				q_enqueue(q, &(b_node_tmp->left));
			}
...
{
	__b_tree_print_levelorder(b_tree, print_data);
}

static void
__b_tree_fprint_levelorder(FILE *output_file ,b_tree_t *b_tree, 
						   void (*print_data)(FILE*, void *))
{
	...
	while (!q_is_empty(q)) {
		...
		for (int i = 0; i < nivel; i++) {
			...
			b_node_tmp = *b_node_tmp_addr;
			q_dequeue(q);
			print_data(output_file, b_node_tmp->data);
			if (b_node_tmp->left) {
				q_enqueue(q, &(b_node_tmp->left));
			}
...
void b_tree_fprint_levelorder(FILE *output_file, b_tree_t *b_tree,
							  void (*print_data)(FILE*, void *))
{
	__b_tree_fprint_levelorder(output_file, b_tree, print_data);
}
